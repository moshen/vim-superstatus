...
if exists('g:loaded_fugitive') && g:loaded_fugitive == 1
	" Show git branch
	" Requires: Fugitive
	let s:stl .= "[#Branch] %(%{substitute(fugitive#statusline(), 'GIT(\\([a-z0-9\\-_\\./:]\\+\\))', '├→ \\1', 'gi')}[#BranchS] [>] %)"
endif
...
" File type
let s:stl .= "[CUR][#Separator][<] \[FT\] [#FileType]%{strlen(&ft) ? &ft : 'n/a'} [/CUR]"

" Line/column/virtual column, Line percentage
let s:stl .= "[#LinePercentS][<<][#LinePercent] %p%% "

" Line/column/virtual column, Line percentage
let s:stl .= "[#LineNumberS][<<][#LineNumber] \[LN\] %l[#LineColumn]:%c%V "
...
call b:SuperstatusRegister('COMPATIBLE', 'COMPATIBLE', s:stl)
